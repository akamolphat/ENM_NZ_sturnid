# Updated humboldt source code
#
# 1. Remove dependency on ks and tcltk package. ks is required required for 
#    *_SP-OVERLAP.pdf plots which are not necessary.
#    The plot can be made locally post-hoc as well.
#
#    The tcltk package is required misc3d package, a prerequisite for ks 
#    (and the reason for problems). The package is used for making 
#    progress bars. All codes involving making progress bars were 
#    hashed. All codes involving the following functions:
#
#    winProgressBar
#    tkProgressBar
#
# 2. Some functions are called via humboldt::function_name. This 
#    assumes that humboldt was installed as a package which was not
#    the case. This was altered so that the functions are called from
#    this source code.
#
# 3. The following line of codes also assumes that the humboldt 
#    package were installed as a package which was not the case. The 
#    following were altered as follows:
#
...
#
#    This particular code occurred in 3 locations in the script, 
#    once in the humboldt.equivalence.stat, twice in 
#    humboldt.background.stat function.
#
# 4. Remove the calculation of PNTI. This is to speed up the process
#    but also because I found the PNTI index unreliable as I keep 
#    getting unrealistic PNTI values. I believe that this may be due
#    to the nature of the variables I used (non-normal, bounded).
#
#
# 
# Define path to this script
# List of librarires loaded here manually ---------------------------
library(ade4)
library(sp)
library(raster)
...
humboldt.pnt.index <- function(glob.g, glob.s, sp, R = 100, kern.smooth = 1) {
  ...
  if (ncol(glob.g) == 2) {
    ...
    spr <- data.frame(cbind((sp[, 1] - xmin)/abs(xmax - xmin), (sp[, 2] - ymin)/abs(ymax - 
                                                                                      ymin)))
    print("Creating object of SpatialPixelsDataframe")
    mask <- ascgen(SpatialPoints(cbind((1:R)/R, (1:R)/R)), nrcol = R - 2, count = FALSE)
    print("Creating kernel density based on species occurence points")
    sp.dens <- kernelUD(SpatialPoints(spr[, 1:2]), h = Kern.SmZ, grid = mask, kern = "bivnorm")
    sp.dens <- raster(xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, matrix(sp.dens$ud, 
                                                                             nrow = R))
    print("Creating kernel density based on background points")
    glob.s.dens <- kernelUD(SpatialPoints(glob.sr[, 1:2]), grid = mask, kern = "bivnorm")
    print("Grid the kernel density of background points into a raster")
    glob.s.dens <- raster(xmn = xmin, xmx = xmax, ymn = ymin, ymx = ymax, matrix(glob.s.dens$ud, 
                                                                                 nrow = R))
    x <- seq(from = min(glob.g[, 1]), to = max(glob.g[, 1]), length.out = R)
    y <- seq(from = min(glob.g[, 2]), to = max(glob.g[, 2]), length.out = R)
    print("Extracting values from global kernel density raster at background points PC1/PC2")
    glob.sr <- extract(glob.s.dens, glob.s)
    Z.th <- quantile(glob.sr, th.env)
    glob.s.dens[glob.s.dens < Z.th] <- 0
    Z <- glob.s.dens * nrow(glob.s)/cellStats(glob.s.dens, "sum")
    print("Extracting values from species kernel density raster at occurrence points PC1/PC2")
    spr <- extract(sp.dens, sp)
    z.th <- quantile(spr, th.sp)
    sp.dens[Z == 0] <- 0
    sp.dens[sp.dens < z.th] <- 0
    ...
    z.uncor2 <- matrix(z.uncor, nrow = R, ncol = R, byrow = F)
    ratio=0
    print("Making countourLines of 10% (15? To be updated) kernel density isopleth of the accessible environment")
    ...
    print("Making countourLines of 5% kernel density isopleth of the species environment")
    ...
    sBf=sBsp+sBenv
    print("Calculating PNTI")
    borders = gDifference(
      as(sBf,"SpatialLines"),
      as(gUnaryUnion(sBf),"SpatialLines"),
      byid=TRUE)
    print("Calculating borders lengthLine")
    print("borders extent:")
    ext(borders)
    #    bordersL<-lengthLine(borders)
    bordersL <- gLength(borders)
    print("Calculating sBsp lengthLine ")
    print("sBsp extent:")
    ext(sBsp)
    #   bordersF<-lengthLine(sBsp)
    bordersF <- gLength(sBsp)
...
humboldt.background.stat<- function(g2e, rep = 100, sim.dir = 1, env.reso, kern.smooth = 1, R = 100,
  ...
  if (force.equal.sample==F){
    ...
    #number of cores attributed for the permutation test
    if (ncores > 1){
      print("Running background.iter.e in parallelisation manner; print here for debugging purposes. force.equal.sample == F")
      cl <- makeCluster(ncores)  #open a cluster for parallelization
      clusterExport(cl=cl, c("z1", "scores.env12a", "scores.env2a", "scores.sp2a", "scores.env2c", "Rana.x", "Rana.y"), envir=environment())
      # invisible(clusterEvalQ(cl, library(humboldt)))
      invisible(clusterEvalQ(cl, source("/nesi/nobackup/uoa02613/A_Chapter5/scripts/humboldt70_custom.R")))
...
background.iter.r<- function(repi, z1, scores.env12a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT) {
  ...
  if (nsp2r > 4) {
    z2.ran <- humboldt.grid.espace(scores.env12a[1:2], scores.env2a[1:2], sp2rPCAs1, R = Rin)
    o.ot <- humboldt.niche.similarity(z1,z2.ran, correct.env = inE, nae = "YES", thresh.espace.z = threshinZ)
...
background.iter.e<- function(repi, z1, scores.env12a, scores.env2a, scores.sp2a, scores.env2c, Rana.x, Rana.y, env.reso.sim, run.silent.bak, inE, Rin, ncores, userOS, threshinZ, kern.smoothinZ, pb, repT) {
  ...
  if (nsp2r >= nsp2t) {
    ...
    z2.ran <- humboldt.grid.espace(scores.env12a[1:2], scores.env2a[1:2], sp2rPCAs2, R = Rin)
    o.ot <- humboldt.niche.similarity(z1,z2.ran, correct.env = inE, nae = "YES", thresh.espace.z = threshinZ)
    # if (ncores == 1 & run.silent.bak == F & userOS==1){setWinProgressBar(pb, repi, title=paste("Background Test: iteration",repi,"(of", repT,"),", round(((repi/repT)*100), 1), "% done"))}
    # if (ncores == 1 & run.silent.bak == F & userOS==2){setTkProgressBar(pb, repi, title=paste("Iteration",repi,"(of", repT,")"))}
    #names(sim.o) <- c("D", "I", "nDg", "nDl", "nIg", "nIl", "n.pts", "x.shift", "y.shift")
...
humboldt.doitall <- function(inname = "DoItAll", env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units = "dd", env.reso, 
  ...
  # r3[cells] <- df[, 3]
  # r3 <- try(update(r3, v = df[,3], cell = cells))
  ...
  #   r3[cells] <- df[, 3]
  # }
  rerr <- try(r3[cells] <- df[,3])
  if (inherits(rerr, "try-error")){
    r3 <- writeRaster(r3, rasterTmpFile(), overwrite=TRUE)
    r3 <- try(update(r3, v = df[,3], cell = cells))
  }
  ...
  # r4[cells] <- df[, 3]
  # r4 <- try(update(r4, v = df[,3], cell = cells))
  ...
  #   r4[cells] <- df[, 3]
  # }
  rerr <- try(r4[cells] <- df[,3])
  if (inherits(rerr, "try-error")){
    r4 <- writeRaster(r4, rasterTmpFile(), overwrite=TRUE)
    r4 <- try(update(r4, v = df[,3], cell = cells))
  }
  ...
  # r1[cells] <- df[, 4]
  # r1 <- try(update(r1, v = df[,4], cell = cells))
  ...
  #   r1[cells] <- df[, 4]
  # }
  rerr <- try(r1[cells] <- df[,4])
  if (inherits(rerr, "try-error")){
    r1 <- writeRaster(r1, rasterTmpFile(), overwrite=TRUE)
    r1 <- try(update(r1, v = df[,4], cell = cells))
  }
  ...
  # r2[cells] <- df[, 4]
  # r2 <- try(update(r2, v = df[,3], cell = cells))
  ...
  #   r2[cells] <- df[, 4]
  # }
  rerr <- try(r2[cells] <- df[,4])
  if (inherits(rerr, "try-error")){
    r2 <- writeRaster(r2, rasterTmpFile(), overwrite=TRUE)
    r2 <- try(update(r2, v = df[,4], cell = cells))
  }
  ...
  options(warn = 0)
  dev.off()
  print("Close *_ENV3.pdf")
  
  ###############################################################
  # calculation of occurrence density and test of niche equivalency and background
  print("Calculating humboldt.equivalence.stat -> a")
  ...
  print("Calculating humboldt.background.stat -> b")
  ...
  print("Calculating humboldt.background.stat -> b2")
  ...
  # print("Estimating PNTI")
  # print(dim(scores.env12a)[1])
  # if (dim(scores.env12a)[1] > 500000){
  #   print("Subsampling global background points for PNTI calculation if there were more than 500,000 points due to crashes")
  #   set.seed(123)  # For reproducibility
  #   scores.env12a.sub <- scores.env12a[sample(nrow(scores.env12a), 100000), 1:2] 
  # } else {
  #   scores.env12a.sub <- scores.env12a[1:2]
  # }
  # if (dim(scores.env1a)[1] > 500000){
  #   print("Subsampling sp1 accessible areas for PNTI calculation if there were more than 500,000 points due to crashes")
  #   set.seed(123)  # For reproducibility
  #   scores.env1a.sub <- scores.env1a[sample(nrow(scores.env1a), 100000), 1:2] 
  # } else {
  #   scores.env1a.sub <- scores.env1a[1:2]
  # }
  # if (dim(scores.env2a)[1] > 500000){
  #   print("Subsampling sp2 accessible areas for PNTI calculation if there were more than 500,000 points due to crashes")
  #   set.seed(123)  # For reproducibility
  #   scores.env2a.sub <- scores.env2a[sample(nrow(scores.env2a), 100000), 1:2] 
  # } else {
  #   scores.env2a.sub <- scores.env2a[1:2]
  # }
  # print("****************")
  # print("Species 1:")
  # pnt1<- humboldt.pnt.index(scores.env12a.sub, scores.env1a.sub, scores.sp1a[1:2], kern.smooth= kern.smoothinZ, R = Rin)
  # rm(scores.env1a.sub)
  # gc()
  # print("Species 2:")
  # pnt2<- humboldt.pnt.index(scores.env12a.sub, scores.env2a.sub, scores.sp2a[1:2], kern.smooth= kern.smoothinZ, R = Rin)
  # rm(scores.env12a.sub, scores.env2a.sub)
  # gc()
...
humboldt.doitall.skip2SPP <- function(inname = "DoItAll", env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units = "dd", env.reso, 
                                      reduce.env = 2, reductype = "PCA", non.analogous.environments = "NO", nae.window=5, env.trim= T, env.trim.type="MCP", trim.mask1, trim.mask2, trim.buffer.sp1 = 200, trim.buffer.sp2 = 200, 
                                      color.ramp=1, correct.env = T, pcx=1, pcy=2, col.env = e.var, e.var, R = 100, kern.smooth = 1, 
                                      e.reps = 100, b.reps = 100, b.force.equal.sample=F, nae = "YES", 
                                      thresh.espace.z = 0.0001, p.overlap = T, p.boxplot = F, p.scatter = F, run.silent = F, ncores=1) {
