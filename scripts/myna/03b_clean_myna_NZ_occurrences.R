# 03b_clean_myna_NZ_occurrences.R
#
# NZ ONLY!!!!
#
...
sf_NZshp <- st_read(dsn= path.expand("data/myna/NZ_calibration_area/NZ_calibration_area.shp"))
...
# Make quick plot
png("results/myna/occurrences/gbif_NZ_range_1971-2020.png", width = 11.7, height = 8.3, res = 300, units = "in")
# plot(st_geometry(sf_NZshp), col = "green", border = "green", xlim = c(45, 124), ylim = c(0, 55))
plot(st_geometry(sf_NZshp))
...
#
sf_NZInPolyNoGeom <- st_drop_geometry(sf_NZInPoly)
# Only store the min and max year of record from the particular location at 0.05 decimal degrees
df_minYr <- setDT(sf_NZInPolyNoGeom)[ , .SD[which.min(year)], by = list(decimalLatitude_05, decimalLongitude_05)]  # return record with min year
df_maxYr <- setDT(sf_NZInPolyNoGeom)[ , .SD[which.max(year)], by = list(decimalLatitude_05, decimalLongitude_05)]  # return record with max year
df_minMaxYr <- unique(rbind(df_minYr, df_maxYr))  # remove repeated records (cases where min = max year)
...
fwrite(df_NZInPoly, file = "data/myna/filtered_occurrences/gbif_NZ_1971-2020_with_cluster.csv", quote = F)
