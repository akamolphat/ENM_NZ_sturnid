# 02_myna_clip_env_calibration_area.R
#
...
# library(rgdal)
# Read in shapefiles ------------------------------------------------#####
v_cal <- vect(path.expand("data/myna/native_calibration_area/native_calibration_area.shp"))
v_plot <- vect(path.expand("data/myna/native_plot_area/native_plot_area.shp"))
...
df_varScaleOffset$scale[24:27] <- 1
...
for (i in 1:(dim(df_varScaleOffset)[1])){
  ...
  terra::writeRaster(r_cropMask, paste("data/myna/native_calibration_area/", names(r_cropMask), "_cropped_masked.tif", sep = ""), overwrite = T)
  # Quick plot of all cropped/masked layers ---------------------------#####
  outputPNG <- paste("data/myna/native_calibration_area/", varName, "_cropped_masked.png", sep = "")
  png(filename = outputPNG, width = 11.7, height = 8.3, units = "in", res = 300)
  ...
  if (varName == "pop_dens_2000"){
    ...
    terra::writeRaster(log_pop_dens, paste("data/myna/native_calibration_area/", names(log_pop_dens), "_cropped_masked.tif", sep = ""), overwrite = T)
    outputPNG <- paste("data/myna/native_calibration_area/", names(log_pop_dens), "_cropped_masked.png", sep = "")
    png(filename = outputPNG, width = 11.7, height = 8.3, units = "in", res = 300)
