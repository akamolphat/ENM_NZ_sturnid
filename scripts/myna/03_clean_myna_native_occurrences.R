# 03_clean_myna_native_occurrences.R
#
# This script clean occurrences data, reduce the dataset to only the native range
# and also performed spatial
# thinning in order to reduce the dataset to only the relavent points in the 
#
...
df_gbif <- fread("data/myna/raw_occurrences/0378113-210914110416597.csv", quote = "", encoding = "UTF-8")
# Subset dataset for only the native range ---------------------------#####
df_gbif <- df_gbif %>%
  filter(decimalLatitude < 55 & decimalLatitude > 0) %>%
  filter(decimalLongitude > 45 & decimalLongitude < 124)
...
gc()
# Make quick plots
# dir.create("results/myna/occurrences/",showWarnings = F, recursive = T)
# png("results/myna/occurrences/gbif_nativerange_1971-2020.png", width = 11.7, height = 8.3, res = 300, units = "in")
# plot(st_geometry(sf_coastline), xlim = c(45, 124), ylim = c(0, 55))
# points(x = df_gbif1971_2020$decimalLongitude,
#        y = df_gbif1971_2020$decimalLatitude)
# 
# dev.off()

# 9. Records outside the region of interest -------------------------#####
# 9.a. Native range calibration area --------------------------------#####
...
sf::sf_use_s2(FALSE)
sf_natInPoly <- st_filter(sf_gbif1971_2020, sf_polyClip) 
# Make quick plot
png("results/myna/occurrences/gbif_nativerange_1971-2020.png", width = 11.7, height = 8.3, res = 300, units = "in")
plot(st_geometry(sf_polyClip), col = "green", border = "green", xlim = c(45, 124), ylim = c(0, 55))
plot(st_geometry(sf_coastline), xlim = c(45, 124), ylim = c(0, 55), add = T)
points(x = df_gbif1971_2020$decimalLongitude,
       y = df_gbif1971_2020$decimalLatitude, col = "red")
plot(st_geometry(sf_natInPoly), col = "blue", add = T)
dev.off()
...
#
# I tried with 2DP but the memory required is very large and crashes alot
#
# df_minYr <- setDT(ptsv2inpolygon@data)[ , .SD[which.min(year)], by = list(decimalLatitude2DP, decimalLongitude2DP)]  # return record with min year
# df_maxYr <- setDT(ptsv2inpolygon@data)[ , .SD[which.max(year)], by = list(decimalLatitude2DP, decimalLongitude2DP)]  # return record with max year
# df_minMaxYr <- unique(rbind(df_minYr, df_maxYr))  # remove repeated records (cases where min = max year)
# df_minMaxYrUnique <- unique(df_minMaxYr[,c("species", "decimalLongitude2DP", "decimalLatitude2DP")])
# pts <- SpatialPointsDataFrame(coords = df_minMaxYrUnique[,c("decimalLongitude2DP", "decimalLatitude2DP")],
#                               data = df_minMaxYrUnique)
#
sf_natInPolyNoGeom <- st_drop_geometry(sf_natInPoly)
# Only store the min and max year of record from the particular location at 0.05 decimal degrees
df_minYr <- setDT(sf_natInPolyNoGeom)[ , .SD[which.min(year)], by = list(decimalLatitude_05, decimalLongitude_05)]  # return record with min year
df_maxYr <- setDT(sf_natInPolyNoGeom)[ , .SD[which.max(year)], by = list(decimalLatitude_05, decimalLongitude_05)]  # return record with max year
df_minMaxYr <- unique(rbind(df_minYr, df_maxYr))  # remove repeated records (cases where min = max year)

# Create a table of just the unique coordinates for performing distance based clustering
df_minMaxYrUnique <- unique(df_minMaxYr[,c("species", "decimalLongitude_05", "decimalLatitude_05")])
...
# Distance with a 10km and 100km threshold  
chc.d10 <- cutree(chc, h=10000) 
chc.d16 <- cutree(chc, h=16000) 
chc.d20 <- cutree(chc, h=20000) 
chc.d100 <- cutree(chc, h=100000) 

# Join results to points
df_minMaxYrUnique <- data.table(df_minMaxYrUnique, Clust10km=chc.d10, Clust16km=chc.d16, Clust20km=chc.d20, Clust100km=chc.d100)

df_natInPoly <- merge(x = sf_natInPoly, 
                      y = df_minMaxYrUnique, 
                      by = c("species", "decimalLatitude_05", "decimalLongitude_05"))

# 9.a.ii. Output native filtered points with defined localities -----#####
fwrite(df_natInPoly, file = "data/myna/filtered_occurrences/gbif_native_1971-2020_with_cluster.csv", quote = F)
