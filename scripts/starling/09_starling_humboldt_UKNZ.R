...
inputcsv <- "data/starling/env_var_df/UKNZ_1km_var_with_thinned_occ.csv"
...
sum(df_comb$range == "UK")
sum(df_comb$range == "UK")/sum(df_comb$range == "NZ")

# Extract environmental variables -----------------------------------
ls_axisCol <- c("x", "y", ls_var)

# Extract variables at species occurrences
scores.sp.nat <- data.frame(df_comb[df_comb$range == "UK" & df_comb$occurrences == 1, ..ls_axisCol])  # UK occurrences
scores.sp.inv <- data.frame(df_comb[df_comb$range == "NZ" & df_comb$occurrences == 1, ..ls_axisCol])  # invasive occurrences

# Calibration area
scores.clim.nat <- data.frame(df_comb[df_comb$range == "UK", ..ls_axisCol])  # UK occurrences
scores.clim.inv <- data.frame(df_comb[df_comb$range == "NZ", ..ls_axisCol])  # NZ occurrences
...
ls_spXY <- c("range", "x", "y")
sp1 <- data.frame(df_comb[df_comb$range == "UK" & df_comb$occurrences == 1, ..ls_spXY]) %>%  # UK distribution
  rename(sp = range)
sp2 <- data.frame(df_comb[df_comb$range == "NZ" & df_comb$occurrences == 1, ..ls_spXY]) %>%  # NZ distribution
  rename(sp = range)
...
dir.create(paste("results/starling/humboldt/UKNZ/", version, sep = ""), showWarnings = F, recursive = T)
if (comp_area == "full"){
  full <- humboldt.doitall(inname = paste("results/starling/humboldt/UKNZ/", version, "/full_", reps, "reps", sep = ""),
  ...
  save(full, file = paste("results/starling/humboldt/UKNZ/", version, "/full_", reps, "reps_results.Rdata", sep = ""))
  ...
  ae <- humboldt.doitall(inname =  paste("results/starling/humboldt/UKNZ/", version, "/ae_", reps, "reps", sep = ""),
                         env1 = scores.clim.nat,
                         env2 = scores.clim.inv,
                         sp1=sp1, sp2=sp2,
  ...
  save(ae, file = paste("results/starling/humboldt/UKNZ/", version, "/ae_", reps, "reps_results.Rdata", sep = ""))
