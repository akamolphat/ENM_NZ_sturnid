# 06_starling_env_correlation.R
# 
# Explore correlations between environmental variables for 
# starling variable selection
#
...
# Read in data ------------------------------------------------------#####
df <- fread(file = "data/starling/env_var_df/NativeNZ_1km_varSubsampled_with_thinned_occ.csv")
...
if (vargrp == "all"){
  ...
  mat_cor_a <- df %>%
    dplyr::filter(occurrences == 1) %>%
    dplyr::select(c(bio1, bio3, bio5, bio12, bio15, gsl, gsp, gst, gdd5, bio10, pop_dens_2000, log_pop_dens, tree_cover_pct, dem)) %>%
    as.matrix() %>%
    rcorr(type = cormethod)
} else if (vargrp == "subset2"){
  mat_cor_a <- df %>%
    dplyr::filter(occurrences == 1) %>%
    dplyr::select(c(bio3, bio5, bio12, bio15, gsl, gdd5, pop_dens_2000, tree_cover_pct, dem)) %>%
    as.matrix() %>%
    rcorr(type = cormethod)
...
mat_cor_c <- df %>%
  dplyr::filter(range == "native") %>%
  dplyr::filter(is.na(occurrences)) %>%  # This is needed for the subsampled dataset 
...
dir.create(path = "results/starling/env_sub_correlation", recursive = T, showWarnings = F)
print(paste("Output ", cormethod, "'s correlation plot", sep = ""))
pdf(paste("results/starling/env_sub_correlation/", vargrp, "_", cormethod, "_coef.pdf", sep = ""))
...
xlsx_file <- paste("results/starling/env_sub_correlation/", vargrp, "_", cormethod, "_coef.xlsx", sep = "")
