# 02d_starling_clip_future_NZ_area.R
#
...
dir.create("data/starling/future_projections/NZ/", recursive = T, showWarnings = F)
for (i in 1:(dim(df_varScaleOffset)[1])){
  ...
  terra::writeRaster(r_cropMask, paste("data/starling/future_projections/NZ/", varName, "_cropped_masked_", sspversion, ".tif", sep = ""), overwrite = T)
  # Quick plot of all cropped/masked layers ---------------------------#####
  outputPNG <- paste("data/starling/future_projections/NZ/", varName, "_cropped_masked_", sspversion, ".png", sep = "")
