...
# Formula for GAM
myGAMform <- formula(paste("starling ~", paste(paste0("s(", ls_varnames, ")"), collapse = " + ")))
# Subsampling for RF
...
# Project -----------------------------------------------------------
## Read tiff files --------------------------------------------------
ls_varfile <- ls_varnames
ls_varfile[ls_varfile == "pop_dens"] <- "pop_dens_2000"
if (range == "nativeNZ"){
  rangeshp <- "comb"
} else {
  rangeshp <- range
}
varpath <- c(paste("data/starling/", rangeshp, "_calibration_area/", ls_varfile, "_cropped_masked.tif", sep = ""))
