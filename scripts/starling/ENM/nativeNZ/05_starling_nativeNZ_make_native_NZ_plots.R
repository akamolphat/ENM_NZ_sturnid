# plot_starling_cross_projections.R ---------------------------------
# 
# This script plots projections and cross-projections for the starling data
#
...
nativerunName <- "native_fullBIOMODv1"
nativeNZrunName <- "nativeNZ_fullBIOMODv1"
mod <- "MAXENT.Phillips"
spfoldername <- "S_vulgaris_distribution_map"

# Read in shape files -----------------------------------------------
sf_NZ <- st_read("data/starling/NZ_calibration_area/starling_NZ_calibration_area.shp")
sf_nat <- st_read("data/starling/native_calibration_area/starling_native_calibration_area.shp")
...
load(paste("data/starling/BIOMOD/nativeNZ/nativeNZ_myBiomodData.Rdata", sep = ""))
...
# 3A. NZ binary, P5
# 3B. NZ binary, P10
pdf_nativeNZv1_NZ <- "results/starling/BIOMOD/nativeNZ/nativeNZv1_NZ_map_PA2.pdf"
...
plot(st_geometry(sf_starInv), add = T, lwd = 0.05)
patternLayer(sf_starInv, "diamond", density = 2, add = T)
# 1B
plot(terra::crop(rast(rast_proj_natNZ), ext(sf_NZ)), axes = F, main = "Binary, max sens. + spec. threshold")
plot(st_geometry(sf_NZ), add = T, lwd = 0.05)
plot(st_geometry(sf_starInv), add = T, lwd = 0.05)
patternLayer(sf_starInv, "diamond", density = 2, add = T)
title("v1 nativeNZ full model PA2, NZ", outer = T)
# 2A
...
pdf_nativeNZv1_nat <- "results/starling/BIOMOD/nativeNZ/nativeNZv1_native_map_PA2.pdf"
...
plot(st_geometry(sf_starNat), add = T, lwd = 0.05)
patternLayer(sf_starNat, "diamond", density = 2, add = T)
# 1B
plot(terra::crop(rast(rast_proj_natNZ), ext(sf_nat)), axes = F, main = "Binary, max sens. + spec. threshold")
plot(st_geometry(sf_map), add = T, lwd = 0.05)
plot(st_geometry(sf_starNat), add = T, lwd = 0.05)
patternLayer(sf_starNat, "diamond", density = 2, add = T)
title("v1 nativeNZ full model PA2, native range", outer = T)
# 2A
