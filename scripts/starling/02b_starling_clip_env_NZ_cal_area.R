...
# library(rgdal)
# Read in shapefiles ------------------------------------------------#####
v_cal <- terra::vect(path.expand("data/starling/NZ_calibration_area/starling_NZ_calibration_area.shp"))
...
for (i in 28:(dim(df_varScaleOffset)[1])){
  ...
  terra::writeRaster(r_cropMask, paste("data/starling/NZ_calibration_area/", names(r_cropMask), "_cropped_masked.tif", sep = ""), overwrite = T)
  # Quick plot of all cropped/masked layers ---------------------------#####
  outputPNG <- paste("data/starling/NZ_calibration_area/", varName, "_cropped_masked.png", sep = "")
  ...
  if (varName == "pop_dens_2000"){
    ...
    terra::writeRaster(log_pop_dens, paste("data/starling/NZ_calibration_area/", names(log_pop_dens), "_cropped_masked.tif", sep = ""), overwrite = T)
    outputPNG <- paste("data/starling/NZ_calibration_area/", names(log_pop_dens), "_cropped_masked.png", sep = "")
    png(filename = outputPNG, width = 11.7, height = 8.3, units = "in", res = 300)
